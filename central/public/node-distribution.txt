...
6. Use **Panel & console readiness** on the Node updates page to check every enabled node and each game server in one pass: Cloudflare connection, gateway reachability, the console socket bypass (probed live without Access credentials), installed release, both DNS records for each panel hostname, and each container's management-network membership and loopback-only administrative ports. The check itself changes nothing; every finding that needs attention carries its own repair control: recreate a server container (restarts that game), start a stopped server, provision or re-check a panel hostname, create the console bypass application immediately, queue a node update, or a link to the page where the fix is made. Each repair re-runs the check when it finishes. The page never rebuilds itself underneath the results. Nodes on releases without readiness support report only hostname checks.
