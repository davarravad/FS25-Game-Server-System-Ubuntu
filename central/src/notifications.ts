...
export async function notifications(db:D1Database,userId:string,gameAdmin=false):Promise<Notice[]> {
  ...
  for(const release of releases)notices.push({id:'release:'+release.version,title:'Release available: '+release.version,detail:'Open a node to review its installed version and available update.',href:'/',severity:'info',ts:release.created});
  if(gameAdmin){
