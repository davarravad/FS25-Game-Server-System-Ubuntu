...
test('management policy rejects unsafe methods, viewer access and oversized uploads',async()=>{
  ...
  const invoke=(op:string,method:string,role:string,body?:BodyInit)=>management(new Request('https://farmservers.sargentweb.com/api/manage?node=node-1&operation='+op,{method,body}),{user_id:'123456789012345678',role},gateway,async()=>{},async()=>null);
...
test('main-site management authenticates, checks CSRF, fixes gateway target and strips cookies',async()=>{
  ...
  let serverAccessHost:string|null='192.168.1.51';
  const mf=new Miniflare(convertV4MiniflareOptions({workers:[{name:'management',modules:true,scriptPath:'dist/index.js',compatibilityDate:'2026-09-09',compatibilityFlags:['nodejs_compat'],d1Databases:['DB'],bindings:{APP_ORIGIN:origin,NODE_GATEWAYS:JSON.stringify({'node-1':{origin:'https://origin-node.sargentweb.com',token:'a'.repeat(64),accessClientId:'client',accessClientSecret:'secret'}})},outboundService:async req=>{
    ...
    if(url.searchParams.get('operation')==='server')return Response.json({ok:true,server:{instance_id:'game-1',sftp_port:2222},secrets:{secrets:{}},access:{host:serverAccessHost,sftp_url:serverAccessHost?'sftp://'+serverAccessHost+':2222':null}});
    return Response.json({ok:true},{headers:{'Set-Cookie':'node-session=private','X-Central-Token':'private'}});
  }}]}));
  try{
    const db=await mf.getD1Database('DB');for(const file of ['0001_control_plane.sql','0013_discord_avatar.sql','0007_blocked_users.sql','0008_node_gateways.sql','0018_node_public_ip.sql']){const sql=await readFile('migrations/'+file,'utf8');for(const statement of sql.split(';').map(s=>s.trim()).filter(Boolean))await db.prepare(statement).run();}
    ...
    await db.prepare('UPDATE nodes SET enabled=1 WHERE id=?').bind('node-1').run();
    await db.prepare('UPDATE nodes SET detected_public_ip=? WHERE id=?').bind('15.204.86.248','node-1').run();
    let serverResponse=await mf.dispatchFetch(origin+'/api/manage?node=node-1&operation=server',{headers});
    assert.equal(serverResponse.status,200);
    let serverData=await serverResponse.json() as {access:{host:string;sftp_url:string}};
    assert.equal(serverData.access.host,'15.204.86.248','A private or unset Access Host/IP is replaced by the node’s detected public IP');
    assert.equal(serverData.access.sftp_url,'sftp://15.204.86.248:2222');
    serverAccessHost='198.51.100.7';
    serverResponse=await mf.dispatchFetch(origin+'/api/manage?node=node-1&operation=server',{headers});
    serverData=await serverResponse.json() as {access:{host:string;sftp_url:string}};
    assert.equal(serverData.access.host,'198.51.100.7','An already-public Access Host/IP is left alone, not replaced by the detected IP');
    assert.equal(serverData.access.sftp_url,'sftp://198.51.100.7:2222');
