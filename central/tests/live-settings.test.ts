...
for(const [type,tab] of [['node','settings'],['server','settings'],['node','create'],['node','connection'],['server','logs'],['setup',''],['status','']])test(`${type} ${tab||'page'} never rebuild automatically after live revisions`,async()=>{
