...
With `CENTRAL_MODE=1` (the default for both the web and agent services), newly rendered instances bind the VNC and noVNC console ports to loopback and join the `fsg-management` network so only the main site can reach the desktop console. The game admin panel (HTTP and TLS), the game port and each server's SFTP port stay public so players, Discord bots and SFTP clients reach them directly; the panel is also served at its public `game-fs25-NNNN.sargentweb.com` HTTPS address. Each instance keeps persistent SFTP host keys under `sftp-keys/`. **Restart** on the site recreates a server's containers so saved settings apply; use **Connection & access → Apply updates to all servers** to recreate every instance on a node.
