...
`CENTRAL_MODE` (default `1` for both the web and agent services) makes **newly rendered** game configurations bind the VNC and noVNC console ports to loopback and attaches the game service to the management Docker network, so the desktop console is reachable only through the main site. The game admin panel (HTTP and TLS ports), the game port and the per-server SFTP port are published publicly: players join directly, Discord bots poll the panel directly, and third-party SFTP clients connect directly; the `game-fs25-NNNN.sargentweb.com` HTTPS address remains the shareable front door for the panel. Each server also keeps a persistent SSH host key pair under `sftp-keys/` in its instance folder, so recreating the SFTP container never triggers changed-host-key warnings. Existing running containers retain their old bindings until their configuration is synchronized and the container is recreated; **Restart** on the site now recreates the container, so it applies saved settings. For a whole node, use **Apply updates to all servers** on the node's **Connection & access** tab (administrators only): it resyncs and force-recreates every enabled server on that node in one step, no SSH required. This interrupts that node's players. Check `docker port INSTANCE_ID`: VNC and noVNC ports must show `127.0.0.1`, while game, web admin and SFTP ports remain public. Docker port publishing can bypass UFW; verify from another computer that the loopback-bound ports are inaccessible directly. Use Docker Engine 28 or newer for loopback-publishing protections.
...
- Direct node origins and raw VNC/noVNC ports cannot bypass central access; the game admin panel is intentionally reachable on its own port as well as through its public HTTPS hostname.
